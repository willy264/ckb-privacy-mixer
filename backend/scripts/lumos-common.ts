...
export async function deployAllBinaries(
    ...
    // Inject enough capacity for ALL binaries. injectCapacity creates a change output at index 0.
    txSkeleton = await commons.common.injectCapacity(
        txSkeleton,
        [address],
        totalCapacity,
    ...
    // Remove the change output that injectCapacity created (we'll rebuild outputs manually)
    txSkeleton = txSkeleton.update('outputs', () => {
        const { List } = require('immutable');
        return List();
    });

    // Add one output per binary contract
    ...
    // payFeeByFeeRate will add a change output and handle fee deduction
    txSkeleton = await commons.common.payFeeByFeeRate(
        txSkeleton,
        [address],
        DEFAULT_FEE_RATE,
    ...
    for (const b of binaries) {
        ...
        console.log(`  ${b.name} => index ${index} (0x${index.toString(16)})`);
    }
    return results;
}
